...
export default class ChecklistNotes extends Component {
    ...
    db.orderByKey()
      ...
      .on("value", snap => {
        ...
        this.setState({ fullList: tempArr[0] }, () => {
          if (this.state.fullList.notes)
            this.setState({ notes: this.state.fullList.notes }, () => {
              let arr = this.state.notes;
              arr.sort((a, b) => {
                let bMonth = b.date.split("/")[0],
                  bDay = b.date.split("/")[1];
                let aMonth = a.date.split("/")[0],
                  aDay = a.date.split("/")[1];
                ...
                if (bMonth === aMonth) {
                  if (bDay === aDay) {
                    return (
                      new Date("1970/01/01 " + bTime) -
                      new Date("1970/01/01 " + aTime)
                    );
                  } else {
                    return bDay - aDay;
                  }
  ...
  toggleEdit = (e, idx) => {
    ...
    this.setState(
      {
        showEditTextarea: !this.state.showEditTextarea,
        editId: idx,
        currentText: txt
      },
      () => {
  ...
  submitEdit = e => {
    ...
    let db = firebase
      .database()
      .ref(`users/${this.state.uid}/${this.state.list}`);
    ...
    this.setState(
      { notes: updatedArr, showEditTextarea: false, editId: null },
      () => {
  ...
  render() {
    return (
      <section className="checklist-notes-section">
        <h2>Notes: </h2>
        <button onClick={this.toggleInput} className="notes-btn">
          {this.state.addNewNote ? "Hide Note Field" : "Add Note"}
        </button>
        {this.state.addNewNote ? (
          <div className="form-group">
            ...
            />
            <button className="add-field-btn" onClick={this.submitData}>
              Save note
            </button>
          </div>
        ) : null}
              ...
              <div
                ...
                className={
                  this.state.editId === idx
                    ? "checklist-note-single active-note"
                    : "checklist-note-single"
                }
              >
                ...
                {this.state.showEditTextarea ? (
                  <div className="edit-group">
                    ...
                    />
                    <button className="add-field-btn" onClick={this.submitEdit}>
                      Save note: {this.state.editId}
                    </button>
                  </div>
                ) : null}
                ...
                <div className="checklist-single-note-stamp">
                  ...
                  <div className="checklist-single-note-btns">
                    <button
                      onClick={e => this.toggleEdit(e, idx)}
                      className="notes-btn"
                    >
                      {this.state.showEditTextarea ? "Close editor" : "Edit"}
                    </button>
                    <button
